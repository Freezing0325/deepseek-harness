...
当约束执行器被挂载时，`bash` 公布配对的 `sandbox_permissions` 和 `justification` 字段。schema 暴露完整的封闭升级词汇，因为有效模式是按会话的；执行仅通过审批通道接受严格宽于该调用有效模式的目标，同级或更窄的请求视为冗余（复用有效模式、不提示——见[幂等非加宽升级修复](../bug-fix/2026-09-09-idempotent-non-widening-sandbox-escalation.zh.md)），无法排名的模式字符串则失败关闭。批准在执行之前解析。`allowed-once` 仅将授权模式盖章到该请求上，而 `rejected`、`cancelled`、`unavailable`、缺失的 approval 服务或缺失的 agent 都以各自不同的结果文本失败关闭。授权不持久化。
...
- 升级字段恰好在已挂载的执行器约束时存在；请求模式等于或低于调用有效模式的请求视为冗余，复用有效模式且不提示任何人（standing `danger-full-access` 策略下，防御性的同级或更窄重试会执行而不是失败），无法排名的模式字符串则以自身文本失败关闭且不提示任何人；没有 ApprovalService 的部署对升级调用失败关闭，对普通调用不影响。
...
- **公布的目标集是静态的，而有效模式是按会话的**（schema 是注册表全局的）——已处于最宽模式的会话仍被提供这些字段。构造上无害：执行时的幂等（而非枚举）是安全边界——同级或更窄的请求复用有效模式且绝不提示任何人，无法排名的模式字符串仍以自身文本失败。
