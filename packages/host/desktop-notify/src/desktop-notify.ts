 ...
 * another window misses it and the agent stalls. This module ports the Claude
 * Code notification recipe (taskbar flash + chime) to the host side: on
 * approval, question, and task-completion events the host spawns a
 * fire-and-forget PowerShell child (win32 only) that flashes the dsh browser
 * taskbar icons and plays a sound. Everything is fire-and-forget — no output
 * is captured, no handle is kept — so an alert can never delay or fail the
 * event channel.
 *
 ...
 * asterisk).
 *
 * @module @deepseek-ai/dsh-host-desktop-notify/desktop-notify
 */
...
/** Settings namespace carrying the desktop-alert switches. */
 ...
 * terminal windows would only disturb unrelated console windows (e.g. a
 * Windows Terminal running an unrelated script).
 */
